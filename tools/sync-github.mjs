...
//   node tools/sync-github.mjs --validate   validates the label manifest and the templates only —
//                                           no targets, no OWNER; this is what CI runs on the kit
...
// An unfilled `<placeholder>` in OWNER or TARGETS is a configuration nobody finished. In the
// generated bash script it is worse than a wrong value: `--repo <org>/<repo>` is parsed as an input
// redirection and fails with "org: No such file or directory", pointing nowhere near the cause.
function rejectPlaceholders({ targets, owner = null }) {
  const values = [];
  if (owner !== null) values.push(["OWNER", owner]);
  for (const target of targets) {
    values.push([`TARGETS name`, target.name], [`TARGETS path`, target.path]);
  }
  for (const [where, value] of values) {
    const placeholder = String(value).match(/<[^<>]+>/);
    if (placeholder) {
      fail(
        `${where} still contains the placeholder "${placeholder[0]}". ` +
          `Replace it with your own value before running — see process/bootstrap-guide.md, step 1.`,
      );
    }
  }
}
...
function renderLabelScript({ labels, targets, owner, pwsh }) {
  rejectPlaceholders({ targets, owner });
  const lines = [];
  if (pwsh) {
    ...
    for (const label of wanted) {
      lines.push(
        `gh label create ${shellQuote(label.name, pwsh)} --color ${shellQuote(label.color, pwsh)} ` +
          `--description ${shellQuote(label.description, pwsh)} --repo ${shellQuote(`${owner}/${target.name}`, pwsh)} --force`,
      );
    }
    lines.push("");
...
function loadTemplates(templateDir, targets) {
  ...
  // A PR-template variant (`PULL_REQUEST_TEMPLATE-<stack>.md`) is never a template in its own
  // right, whether or not a target currently uses it: removing the one target that overrides it
  // must not turn it into an "Issue form" that fails validation or gets distributed.
  const isPrVariant = (f) => /^PULL_REQUEST_TEMPLATE-.+\.md$/.test(f);

  const templates = readdirSync(templateDir)
    .filter((f) => f.endsWith(".yml") || f.endsWith(".md"))
    .filter((f) => !overrideSources.has(f) && !isPrVariant(f))
    .sort();
...
const SELF_TEST_CASES = [
  ...
  {
    name: "a PR-template variant no target uses is neither validated as an Issue form nor distributed",
    run: () =>
      withFixture(
        { templates: { "1-story.yml": issueForm(), "PULL_REQUEST_TEMPLATE-frontend.md": "## Summary\n" } },
        (f) => {
          const r = templatesRun(false)(f);
          const leaked = existsSync(join(f.targets[0].path, ".github", "ISSUE_TEMPLATE", "PULL_REQUEST_TEMPLATE-frontend.md"));
          if (r.error !== null) return `expected success, got error "${r.error}"`;
          return leaked ? "the variant was distributed as an Issue template" : null;
        },
      ),
  },
  {
    name: "unfilled <placeholder> in OWNER is refused instead of rendering the label script",
    run: () =>
      withFixture({ manifest: { states: { labels: [VALID_LABEL] } } }, (f) => {
        const labels = loadLabels(f.manifestPath, f.targets);
        return expectRefusal(/OWNER still contains the placeholder "<organization-or-account>"/)(
          capture(() => renderLabelScript({ labels, targets: f.targets, owner: "<organization-or-account>", pwsh: false })),
        );
      }),
  },
  {
    name: "unfilled <placeholder> in TARGETS is refused before any template is written",
    run: () =>
      withFixture({ templates: { "1-story.yml": issueForm() } }, (f) => {
        const targets = [{ ...f.targets[0], name: "<repo-backend>" }];
        const r = capture((out) => runTemplates({ targets, templateDir: f.templateDir, checkOnly: false, out, revision: () => "self-test" }));
        const written = existsSync(join(f.targets[0].path, ".github", "ISSUE_TEMPLATE", "1-story.yml"));
        const refused = expectRefusal(/TARGETS name still contains the placeholder "<repo-backend>"/)(r);
        return refused ?? (written ? "refused, but a template was written anyway" : null);
      }),
  },
  {
    name: "--repo is quoted, so a value with shell metacharacters stays one argument",
    run: () =>
      withFixture({ manifest: { states: { labels: [VALID_LABEL] } } }, (f) => {
        const labels = loadLabels(f.manifestPath, f.targets);
        const script = renderLabelScript({ labels, targets: f.targets, owner: "o", pwsh: false });
        return script.some((l) => l.includes(`--repo 'o/${f.targets[0].name}' --force`))
          ? null
          : `expected a quoted --repo 'o/${f.targets[0].name}', got: ${script.join(" | ")}`;
      }),
  },
];
...
try {
  // Validates the SOURCE set without touching or naming any target, so it works on the kit itself,
  // where OWNER and TARGETS are still placeholders by design. A target subset naming an unknown
  // label or an override pointing at a missing file is still caught — both are source errors.
  if (VALIDATE) {
    const labels = loadLabels(LABEL_MANIFEST, TARGETS);
    const templates = loadTemplates(TEMPLATE_DIR, TARGETS);
    console.log(`Valid: ${labels.length} labels in the manifest, ${templates.length} templates.`);
    process.exit(0);
  }
